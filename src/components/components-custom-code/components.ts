...
import { commandNameCustomCode, keyCustomCode, typeCustomCode } from "./utils";
		...
		view: {
			...
			onComponentsChange() {
				...
				timedInterval = setTimeout(() => {
					...
					if (
						content.indexOf("<script") >= 0 &&
						opts.placeholderScript
					) {
			...
			onActive() {
				const { model, em } = this;
				em.get("Commands").run(commandNameCustomCode, {
					target: model,
				});
			},
		},
